...
# BUSCAR DO BANCO
# =========================
df_prog = pd.DataFrame(supabase.table("programacao").select("*").execute().data)
df_prod = pd.DataFrame(supabase.table("producao").select("*").execute().data)
df_ent = pd.DataFrame(supabase.table("entregas").select("*").execute().data)


# =========================
# DATAS
# =========================
df_prog["data_pedido"] = pd.to_datetime(df_prog["data_pedido"])
df_prod["data"] = pd.to_datetime(df_prod["data"])
df_ent["data_transacao"] = pd.to_datetime(df_ent["data_transacao"])
...
data_inicio, data_fim = st.date_input(
    "Selecione o período",
    [df_ent["data_transacao"].min(), df_ent["data_transacao"].max()]
)
...
# PRODUÇÃO
# =========================
df_prod["inicial_dt"] = pd.to_datetime(df_prod["data"].astype(str) + " " + df_prod["inicial"].astype(str))
df_prod["final_dt"] = pd.to_datetime(df_prod["data"].astype(str) + " " + df_prod["final"].astype(str))
